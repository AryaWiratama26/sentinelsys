...
class VisualizerRun:
    ...
    def plot_graph(self):
        ...
        while True:
            ...
            ax[0].cla()
            ax[0].plot(self.timestamps, self.ram_data, label="RAM Usage (%)", color='g')
            ax[0].set_title("RAM Usage")
            ax[0].set_ylim(0,100)
            ax[0].legend()
            
            ax[0].cla()
            ax[0].plot(self.timestamps, self.disk_data, label="Disk Usage (%)", color='b')
